...
void print_usage()
{
  ...
  printf("-t topic_name : Specify the topic on which to subscribe. Defaults to joystick.\n");
}
...
public:
  ...
  : Node("listener")
  {
    ...
    auto callback =
	  ...
	  {
		RCLCPP_INFO(this->get_logger(), "I heard: button %i has value %i", msg->buttons.size(), msg->buttons.back());
	  };
	...
	sub_ = create_subscription<sensor_msgs::msg::Joy>(topic_name, callback);
  }
