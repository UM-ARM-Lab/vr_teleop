/****************************************************************************
 *  Copied from image_view
 * https://github.com/ros-perception/image_pipeline/tree/indigo/image_view
 * Edited by Brad Saund brad.saund@gmail.com
 *
...
ros::Time last_record_command_received = ros::Time(0);
std::string encoding;
std::string codec;
std::string record_command_topic;
int fps;
std::string filename;
...
void recordCommandCallback(const std_msgs::String& str_msg)
{
    recording = true;
    last_record_command_received = ros::Time::now();
}
...
std::string makeFilename()
{
    ...
    std::string basename = filename.substr(found);
    std::stringstream ss;
    // ss << ros::Time::now().toNSec() << basename;
    ss << "_" << arc_helpers::GetCurrentTimeAsString() << basename;
    std::string stamped_filename = path + ss.str();
    ROS_INFO("Video recording to %s", stamped_filename.c_str());
...
void callback(const sensor_msgs::ImageConstPtr& image_msg)
{
    ...
    if((ros::Time::now() - last_record_command_received).toSec() > 1.0)
    {
        recording = false;
        g_count = 0;
        if(outputVideo.isOpened())
        {
            ROS_INFO("Wrote video");
            outputVideo.release();
        }
        return;
    }
...
int main(int argc, char** argv)
{
    ...
    local_nh.param("stamped_filename", stamp_filename, false);
    ...
    local_nh.param("record_command_topic", record_command_topic, std::string("/record_command"));
    ...
    ros::Subscriber topic_sub = nh.subscribe(record_command_topic, 1000, recordCommandCallback);
